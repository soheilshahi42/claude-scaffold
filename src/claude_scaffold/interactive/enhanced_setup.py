...
from ..utils.terminal_ui import EnhancedTerminalUI
...
class EnhancedInteractiveSetup:
    ...
    def _load_config(self, config_file: Optional[str]) -> Dict[str, Any]:
        ...
        default_config = {
            "discovery": {"max_questions": 100, "enable_deep_discovery": True},
            "claude": {"timeout_seconds": 300, "max_retries": 3, "chunk_size": 3},
            "ui": {
    ...
    def run(self) -> Dict[str, Any]:
        ...
        if self.config["discovery"]["enable_deep_discovery"]:
            self.ui.show_info(
                "Starting deep discovery phase. I'll ask some questions to better understand your project."
            )
            self._run_discovery_phase(project_info)

        # Step 3: Generate enhanced project structure
        self.ui.show_info(
            "Now I'll generate an enhanced project structure based on our discussion."
        )
    ...
    def _collect_initial_info(self) -> Dict[str, Any]:
        ...
        # Project name
        project_name = self.ui.ask_question("What would you like to name your project?")
        ...
        project_type = self.ui.ask_question(
            "What type of project are you building?", choices=project_types
        )
        ...
        language = self.ui.ask_question(
            "What programming language will you use?", choices=languages
        )
    ...
    def _run_discovery_phase(self, project_info: Dict[str, Any]):
        ...
        while question_count < max_questions:
            ...
            with progress_indicator.claude_thinking(
                f"Generating {category.replace('_', ' ')} question"
            ):
                question = self._generate_contextual_question(
                    project_info, self.discovery_responses, category
                )
        ...
        self.ui.show_success(
            f"Discovery phase complete! Collected {len(self.discovery_responses)} responses."
        )

    def _generate_contextual_question(
        self, project_info: Dict, responses: List[Dict], category: str
    ) -> Optional[str]:
    ...
    def _summarize_discovery(self) -> Dict[str, Any]:
        ...
        for response in self.discovery_responses:
            ...
            if category not in summary:
                summary[category] = []
            summary[category].append({"q": response["question"], "a": response["answer"]})
