...
class DocumentationGenerator:
    ...
    def generate_documentation(self, project_path: Path, project_data: Dict[str, Any]):
        ...
        total_docs += len(project_data.get("modules", [])) * 2  # Each module: CLAUDE.md, TODO.md
        total_docs += sum(
            len(tasks)
            for tasks in self.formatters.organize_tasks_by_module(project_data["tasks"]).values()
        )  # Research docs
        ...
        with ui_manager.step_progress(
                ...
                module_context.update(
                    {
                        ...
                        "module_responsibilities": claude_docs.get(
                            "responsibilities",
                            self.helpers.get_module_responsibilities(module, project_data),
                        ),
            ...
            if module["name"] in module_tasks:
                for task in module_tasks[module["name"]]:
                    research_file = (
                        module_path / "docs" / f"{self.formatters.slugify(task['title'])}.md"
                    )
