...
class DebugLogger:
    ...
    def __init__(self, debug_mode: bool = False, log_file: Optional[Path] = None):
        self.debug_mode = debug_mode or os.getenv("CLAUDE_SCAFFOLD_DEBUG", "").lower() in [
            "true",
            "1",
            "yes",
        ]
    ...
    def error(
        ...
        self.logger.error(
            message, exc_info=error is not None, extra={"session_id": self.session_id}
        )
    ...
    def log_performance(self, operation: str, duration: float, success: bool = True):
        ...
        self.logger.log(
            level,
            message,
            extra={"session_id": self.session_id, "performance": perf_data},
        )
