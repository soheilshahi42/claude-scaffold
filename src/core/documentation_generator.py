...
class DocumentationGenerator:
    ...
    def generate_documentation(self, project_path: Path, project_data: Dict[str, Any]):
        ...
        # Calculate total documentation files
        total_docs = 4  # Root files: CLAUDE.md, GLOBAL_RULES.md, TASKS.md, TODO.md
        total_docs += (
            len(project_data.get("modules", [])) * 2
        )  # Each module: CLAUDE.md, TODO.md
        total_docs += sum(
            len(tasks)
            for tasks in self.formatters.organize_tasks_by_module(
                project_data["tasks"]
            ).values()
        )  # Research docs

        with ui_manager.step_progress(
            "Generating Documentation", total_steps=total_docs
        ) as progress:
            # Prepare context for templates
            context = self.helpers.prepare_template_context(project_data)

            # Generate root CLAUDE.md
            progress.update("Creating root CLAUDE.md")
            ...
            module_tasks = self.formatters.organize_tasks_by_module(
                project_data["tasks"]
            )

            for module in project_data.get("modules", []):
                progress.update(f"Creating documentation for {module['name']} module")
                    ...
                    module_context.update(
                        {
                            ...
                            "module_responsibilities": claude_docs.get(
                                "responsibilities",
                                self.helpers.get_module_responsibilities(
                                    module, project_data
                                ),
                            ),
                            "public_api": claude_docs.get(
                                "public_api", "To be defined during implementation"
                            ),
                            "internal_architecture": claude_docs.get(
                                "architecture", "To be defined during implementation"
                            ),
                            "dependencies": self.formatters.format_dependencies(
                                claude_docs.get("dependencies", [])
                            ),
                            "naming_conventions": self.helpers.get_naming_conventions(
                                project_data
                            ),
                            "file_organization": self.helpers.get_file_organization(
                                module, project_data
                            ),
                            "error_handling": claude_docs.get(
                                "error_handling",
                                self.helpers.get_error_handling(project_data),
                            ),
                            "module_tasks": self.formatters.format_module_tasks(
                                module["name"], module_tasks.get(module["name"], [])
                            ),
                            "testing_strategy": self.helpers.get_testing_strategy(
                                project_data
                            ),
                            "performance_notes": claude_docs.get(
                                "performance", "To be defined based on requirements"
                            ),
                            "security_notes": claude_docs.get(
                                "security", "To be defined based on requirements"
                            ),
                            "example_imports": "specific_function, SpecificClass",
                            "usage_example": self.formatters.format_examples(
                                claude_docs.get("examples", [])
                            ),
                        }
                    )
                else:
                    module_context.update(
                        {
                            "module_name": module["name"],
                            "module_description": module["description"],
                            "module_responsibilities": self.helpers.get_module_responsibilities(
                                module, project_data
                            ),
                            "public_api": "To be defined during implementation",
                            "internal_architecture": "To be defined during implementation",
                            "dependencies": self.helpers.get_module_dependencies(
                                module, project_data
                            ),
                            "naming_conventions": self.helpers.get_naming_conventions(
                                project_data
                            ...
                            "error_handling": self.helpers.get_error_handling(
                                project_data
                            ),
                            "module_tasks": self.formatters.format_module_tasks(
                                module["name"], module_tasks.get(module["name"], [])
                ...
                module_claude = self.templates.get_template(
                    "module_claude", module_context
                )
                (module_path / "CLAUDE.md").write_text(module_claude)

                # Create module TODO.md
                progress.update(f"Creating TODO.md for {module['name']} module")
                module_todos = []
                ...
                if module["name"] in module_tasks:
                    for task in module_tasks[module["name"]]:
                        progress.update(
                            f"Creating research doc: {task['title'][:40]}..."
                        )
                        research_file = (
                            module_path
                            / "docs"
                            / f"{self.formatters.slugify(task['title'])}.md"
                        )
    ...
    def create_todo_file(
        self, file_path: Path, scope: str, scope_description: str, todo_items: List[str]
    ):
        ...
        context = {
            ...
            "status_summary": f"{Icons.CHART} {completed}/{total} tasks completed ({completion_percentage:.0f}%)",
            "todo_items": (
