...
from ..utils.icons import icons, Icons
...
class DocumentationGenerator:
    ...
    def generate_documentation(self, project_path: Path, project_data: Dict[str, Any]):
        ...
        total_docs += sum(len(tasks) for tasks in self.formatters.organize_tasks_by_module(project_data['tasks']).values())  # Research docs
        
        with ui_manager.step_progress("Generating Documentation", total_steps=total_docs) as progress:
            ...
            for module in project_data.get('modules', []):
                ...
                if module['name'] in module_tasks:
                    for task in module_tasks[module['name']]:
                        progress.update(f"Creating research doc: {task['title'][:40]}...")
